...
buttonGet.addEventListener('click', async () => {
  try {
    ...
    document.getElementById('nameMailPhone').innerHTML = `<div>Name:</div><div>Mail:</div><div>Phone:</div>`
    output.innerHTML = `<div class="userName">${users.name}</div>
      <div class="userMail">${users.email}</div>
       <div class="phone">${users.phone}</div>`
    document.getElementById('buttonSavePosition').innerHTML = `<button id="buttonSave" type="submit">Save</button>`
...
const render = (savedUserName) => {
  ...
  nameContainer.innerHTML = `${savedUserName.map(
